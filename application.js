...
function renderStoreDetails(container, template, collection, slug){
    ...
    $.each( item_list , function( key, val ) {
        if ((val.store_front_url).indexOf('missing.png') > -1){
            val.alt_store_front_url = "http://assets.codecloudapp.com/sites/57b338c76e6f642abb010000/image/png/undefined/corelogo.png";
        } else {
            val.alt_store_front_url = getImageURL(val.store_front_url); 
        }
...
function renderJobDetails(container, template, collection, mall_name){
    ...
    $.each( item_list , function( key, val ) {
        ...
        if (start.format("DMY") == end.format("DMY")){
            ...
            val.dates = start.format("MMM DD") + " - " + end.format("MMMM DD")
        }
...
function renderPosts(container, template, collection){
    ...
    $.each( collection , function( key, val ) {
        if (val.image_url.indexOf('missing.png') > -1) {
            val.post_image = "http://assets.codecloudapp.com/sites/57b338c76e6f642abb010000/image/png/undefined/corelogo.pngg";
        } else {
...
function renderPostDetails(container, template, collection){
    ...
    $.each( collection , function( key, val ) {
        if (val.image_url.indexOf('missing.png') > -1) {
            val.post_image = "http://assets.codecloudapp.com/sites/57b338c76e6f642abb010000/image/png/undefined/corelogo.png";
        } else {
            val.post_image = val.image_url;
        }
        if(val.body.length > 100){
